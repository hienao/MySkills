...
当前版本：`1.2.0`（以 [`VERSION`](./VERSION) 为准）。
...
- 为每天生成带编号、真实相对方位、分段距离和交通时间的路径规划图；默认不用箭头重复表达顺序。
...
每日路径图规范位于 `references/route-map-spec.md`，可直接复用的无箭头编号样板位于 `assets/daily-route-map-sample.html`。
